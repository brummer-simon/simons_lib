...
ifeq ($(PROJECT_TYPE), binary)
	...
	$(info Create symlinks to binary "$(BIN_DEBUG)" in "$(INSTALL_BIN_DIR)".)
	$(CMD_LN) $(INSTALL_BIN_DIR)/$(BIN_DEBUG_FULL_VERSION) $(INSTALL_BIN_DIR)/$(BIN_DEBUG_BASENAME)
...
ifeq ($(PROJECT_TYPE), binary)
	...
	$(info Create symlinks to binary "$(BIN_RELEASE)" in "$(INSTALL_BIN_DIR)".)
	$(CMD_LN) $(INSTALL_BIN_DIR)/$(BIN_RELEASE_FULL_VERSION) $(INSTALL_BIN_DIR)/$(BIN_RELEASE_BASENAME)
else ifeq ($(PROJECT_TYPE), libstatic)
...
ifeq ($(PROJECT_TYPE), binary)
	$(ETC_SBIN_DIR)/uninstall_save.sh $(INSTALL_BIN_DIR)/$(BIN_DEBUG_BASENAME)
	$(ETC_SBIN_DIR)/uninstall_save.sh $(INSTALL_BIN_DIR)/$(BIN_DEBUG_FULL_VERSION)
else ifeq ($(PROJECT_TYPE), libstatic)
...
ifeq ($(PROJECT_TYPE), binary)
	$(ETC_SBIN_DIR)/uninstall_save.sh $(INSTALL_BIN_DIR)/$(BIN_RELEASE_BASENAME)
	$(ETC_SBIN_DIR)/uninstall_save.sh $(INSTALL_BIN_DIR)/$(BIN_RELEASE_FULL_VERSION)
else ifeq ($(PROJECT_TYPE), libstatic)
...
help:
	...
	$(info +-----------------+-----------+-----------+-----------+-------------+--------------------------------------+)
	$(info | Project Type:   | binary    | libstatic | libshared | headeronly | Description                          |)
	$(info | Project Config: | dbg | rel | dbg | rel | dbg | rel | dbg | rel  |                                      |)
	$(info +-----------------+-----+-----+-----+-----+-----+-----+------------+--------------------------------------+)
	$(info | build           |  X  |  X  |  X  |  X  |  X  |  X  |  X  |  X   | Build current config                 |)
	$(info | clean           |  X  |  X  |  X  |  X  |  X  |  X  |  X  |  X   | Clean build artefacts (current cfg)  |)
	$(info | exec            |  X  |  X  |     |     |     |     |     |      | Execute build result                 |)
	$(info | exec_debug      |  X  |  X  |     |     |     |     |     |      | Execute build result in debugger     |)
	$(info | test            |     |     |  X  |  X  |  X  |  X  |  X  |  X   | Build and run unittests              |)
	$(info | test_debug      |     |     |  X  |  X  |  X  |  X  |  X  |  X   | Build and run unittests in debugger  |)
	$(info | install         |  X  |  X  |  X  |  X  |  X  |  X  |  X  |  X   | Install build result in host system  |)
	$(info | uninstall       |  X  |  X  |  X  |  X  |  X  |  X  |  X  |  X   | Remove build result from host system |)
	$(info | doc             |  X  |  X  |  X  |  X  |  X  |  X  |  X  |  X   | Create documentation in doc          |)
	$(info | rebuild         |  X  |  X  |  X  |  X  |  X  |  X  |  X  |  X   | Rebuilds current configuration       |)
	$(info | all             |  X  |  X  |  X  |  X  |  X  |  X  |  X  |  X   | Rebuild debug and release config     |)
	$(info | clean_all       |  X  |  X  |  X  |  X  |  X  |  X  |  X  |  X   | Clean all build artefacts            |)
	$(info | help            |  X  |  X  |  X  |  X  |  X  |  X  |  X  |  X   | Prints this help screen              |)
	$(info +-----------------+-----+-----+-----+-----+-----+-----+---------------------------------------------------+)
